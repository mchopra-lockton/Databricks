...
# MAGIC   GoldDimTableName = "BP_NX_REF_MASTER_LOB_MAPPING"
# MAGIC }
...
sourceSilverFolderPath = "Business/MDS2/Mapping_LOB/" +now.strftime("%Y") + "/" + now.strftime("%m")
sourceSilverPath = SilverContainerPath + sourceSilverFolderPath

sourceSilverFile = "Mapping_LOB_" + now.strftime("%Y") + "_" + now.strftime("%m") + "_" + now.strftime("%d") + ".parquet"
...
sourceSilverDF.createOrReplaceTempView("BP_NX_REF_MASTER_LOB_MAPPING")
...
f""" 
SELECT
LineOfBusinessGroup As LINE_OF_BUSINESS_GRP,
LineOfBusinessMaster As MASTER_LINE_OF_BUSINESS,
'{ BatchId }' AS ETL_BATCH_ID,
'{ WorkFlowId }' AS ETL_WRKFLW_ID,
current_timestamp() AS ETL_CREATED_DT,
current_timestamp() AS ETL_UPDATED_DT
from BP_NX_REF_MASTER_LOB_MAPPING
"""
)
display(finalDataDF)
